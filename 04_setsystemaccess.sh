...
# 04_setsystemaccess
#
# Пароль root, порт SSH, публичный ключ для root (ветка freebsd).
...
# Ветка freebsd: выполняется только при VPCONFIGURE_GIT_BRANCH=freebsd.
...
usage() {
  ...
  cat >&2 <<EOF
Настройка доступа: пароль root, порт sshd, ключ в authorized_keys (только ветка freebsd).
...
Пример (vpconnect_install подставляет только нужные флаги):
  export VPCONFIGURE_GIT_BRANCH=freebsd
