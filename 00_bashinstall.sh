...
# Запуск до появления bash: sh ./00_installbash.sh
...
if command -v bash >/dev/null 2>&1; then
  ...
  vp_result_line success "bash уже установлен" "path:${_p}" "version:${_ver}"
  exit 0
fi
...
_p=$(command -v bash)
_ver=$(bash --version | head -n 1 | tr ';' ',')
vp_result_line success "bash установлен" "path:${_p}" "version:${_ver}"
if [ -n "${POST_NOTE:-}" ]; then
  printf 'Note: %s\n' "$POST_NOTE" >&2
fi
