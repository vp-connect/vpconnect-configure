...
Они **не** входят в нумерованную цепочку `00–08` как отдельные шаги, но при успешном **`06_setwireguard.sh`** (в этой ветке только `centos`), если рядом с ним на сервере есть каталог **`wg/`** с этими `*.sh`, скрипт **06** выставляет им **`chmod a+x`** и создаёт **симлинки в `/usr/local/bin/`** (вызов из консоли: `wg.sh help` и т.д.).

В этой ветке все `wg/*.sh` ожидают `VPCONFIGURE_GIT_BRANCH=centos` и завершаются с ошибкой при других значениях.
