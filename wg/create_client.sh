...
# create_client.sh
#
# Создание клиента WireGuard: ключи, запись [Peer] в /etc/wireguard/<iface>.conf, клиентский .conf,
# QR (ansiutf8) в каталог qr. Применение без полного перезапуска: wg syncconf <iface>.
#
...
# В centos-ветке допускается только VPCONFIGURE_GIT_BRANCH=centos.
...
require_centos_branch() {
    ...
    if [[ "$b" != "centos" ]]; then
        echo "Ошибка: create_client.sh в ветке centos поддерживает только VPCONFIGURE_GIT_BRANCH=centos (текущее: ${b:-unset})." >&2
        exit 1
    fi
}
