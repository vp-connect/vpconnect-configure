...
Для текущей **freebsd-ветки репозитория** действует политика: скрипты **`04`–`08`**, каталог **`lib/`** и runtime-скрипты **`wg/`** выполняются только при `VPCONFIGURE_GIT_BRANCH=freebsd` (иначе `result:error`).
Скрипты **`00`–`03`** остаются универсальными для `freebsd|debian|centos`.
Целевая поддержка FreeBSD в этой ветке: **13** и **14**.
...
| **`VPCONFIGURE_DOMAIN`** | Публичное **имя или IP** сервера (FQDN или адрес), подставляется в **`mtproxy.link`**, в настройки VPManage и т.п. Задаётся **`05_setdomain.sh`** или вручную; **нужна** для **`07`** и **`08`** (в этой ветке — freebsd-only). |
...
Первая строка **stdout** всегда начинается с `result:success`, `result:warning` или `result:error`, далее `; message:текст` и при необходимости `; ключ:значение` (например `branch:freebsd`). В `message` символ `;` не использовать. Пояснения и лог — в **stderr**
...
| `04_setsystemaccess.sh` | Пароль root, SSH‑порт, ключ (в этой ветке — только `freebsd`; опционально **`--enable-firewall`**) |
| `05_setdomain.sh` | Hostname / домен |
| `06_setwireguard.sh` | WireGuard (в этой ветке — только `freebsd`; **`--wg-port`**, пути клиентских каталогов, опционально **`--wg-server-private-key-file`**) |
| `07_setmtproxy.sh` | MTProxy (в этой ветке — только `freebsd`; **`--mtproxy-port`**, опционально **`--mtproxy-secret`**, **`--export`**, **`--persist`**) |
| `08_setvpmanage.sh` | VPManage (в этой ветке — только `freebsd`) |
